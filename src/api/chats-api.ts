...
import {CreateChatRequest, ChatUsersRequest, EmptyRequest} from "../services/types";
...
export class ChatsApi {
    ...
    createChat(title: string) {
        const options: Options<CreateChatRequest> = {data: {title}, headers: {"Content-Type": "application/json"}};
    ...
    getUsers(chatId: string) {
        ...
        return chatsAPIInstance.get(`/${chatId}/users`, options)
            .then((data: string) => JSON.parse(data))
            .catch(err => {
                throw err
            });
    ...
    addUsers(data: ChatUsersRequest) {
        const options: Options<ChatUsersRequest> = {data, headers: {"Content-Type": "application/json"}};
